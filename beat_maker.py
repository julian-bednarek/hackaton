...
OUTPUT_FILE = 'WESAD/S2_structured_pop.wav'
...
# --- 4. Main ---
def main():
	data = load_wesad_data(INPUT_FILE)
	if data is None: return

	limit = DURATION_LIMIT_SEC * DATA_SAMPLING_RATE
	ecg = data['signal']['chest']['ECG'].flatten()[:limit]
	emg = data['signal']['chest']['EMG'].flatten()[:limit]

	# Calculate continuous HR
	print("Analyzing Heart Rate for tempo...")
	ecg_clean = nk.ecg_clean(ecg, sampling_rate=DATA_SAMPLING_RATE)
	_, rpeaks = nk.ecg_peaks(ecg_clean, sampling_rate=DATA_SAMPLING_RATE)
	ecg_rate = nk.signal_rate(rpeaks, sampling_rate=DATA_SAMPLING_RATE, desired_length=len(ecg))

	# Generate
	final_song = generate_song_structure(ecg_rate, emg, DATA_SAMPLING_RATE, DURATION_LIMIT_SEC)
	final_song.export(OUTPUT_FILE, format="wav")
	print(f"Pop song saved to {OUTPUT_FILE}")
